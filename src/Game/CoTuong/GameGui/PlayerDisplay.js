...
CoTuong.PlayerDisplay = cc.Node.extend({
    ...
    updateWithPlayer: function(player) {
        ...
        if (player.status == 2) {
            ...
            cc.log(CoTuong.gameLogic.myGameChair);
            if (player.info.gameChair == CoTuong.gameLogic.myGameChair) {
                cc.log("show");
                this.showBtnStandUp();
            } else {
                cc.log("hide");
    ...
    addMoney: function(money, time)           // Effect tien` khi het van' choi
    {
        ...
        this.updateMoney = function(){
            var player = CoTuong.gameLogic.getPlayerByGameChair(this.index);
            if (player)
                this.cash.setString(StringUtility.standartNumber(player.info["money"]));
        };
    ...
    onSittingClick: function(sender, type) {
        if (type == ccui.Widget.TOUCH_ENDED) {
            var pk = new CoTuong.CmdSendRequestSitting();
            pk.putData();
    ...
    onStandUp: function(sender, type) {
        if (type == ccui.Widget.TOUCH_ENDED) {
            var pk = new CoTuong.CmdSendRequestStandUp();
            pk.putData();
            gameWsClient.send(pk);
            pk.clean();
        }
