...
CoTuong.soldierVector = {
    x: [1, 0, 0],
    y: [0, -1, 1]
};

CoTuong.soldierVectorEnemy = {
    x: [-1, 0, 0],
    y: [0, -1, 1]
};
...
CoTuong.startPieceReverse = pieceReverse;

CoTuong.tileWidth = 559 / 8;
CoTuong.tileHeight = 627 / 9;

CoTuong.getPosInMap = function(x, y) {
    return cc.p(CoTuong.tileWidth * y, CoTuong.tileHeight * x);
};

CoTuong.convertPointToPos = function(point) {
    return {x: parseInt(Math.ceil(point.y / CoTuong.tileWidth - 1/2)), y: parseInt(Math.ceil(point.x / CoTuong.tileHeight - 1/2))};
};
