//
BaCay.PlayerDisplay = cc.Node.extend({
    ctor: function() {
        this._super();
        this.handOnCards = [];
        this.baiEndGame = [];
    ...
    initWithCards: function(cards){
        ...
        for(var i = 0; i < cards.length; i++){
            ...
            this.handOnCards[i].setTexture(cc.textureCache.addImage(BaCay.CardSprite.getResource(cards[i])));
        }
    },
    ...
    initPlayerDisplay: function(){
        ...
        if(this.index == 0){
            this.betButtonList = [];
            for(var i = 1; i <=4; i++){
                //this["btnDatCuoc"+i] = ccui.helper.seekWidgetByName(this.panel, "btn_dat_cuoc_" + i);
                ...
                //this.text_dat_cuoc = ccui.helper.seekWidgetByName(this.panel, "text_dat_cuoc");
        ...
        for(i = 1; i <= 3; i++){
            //this["card" + i] = ccui.helper.seekWidgetByName(this.cardPanel, "card" + i);
            this["card" + i] = this.cardPanel.getChildByName("card" + i);
            //cc.log("initPlayerDisplay 17 " + i);
        ...
        //this.uiAvatar = ccui.helper.seekWidgetByName(this.panel, "btnAvatar");
        this.uiAvatar = avatarPanel.getChildByName("btnAvatar");
        //cc.log("initPlayerDisplay 18");
        //this.avatar = ccui.helper.seekWidgetByName(this.panel, "avatar");
        this.avatar = avatarPanel.getChildByName("avatar");

        //cc.log("initPlayerDisplay 19");
        //this.uiName = ccui.helper.seekWidgetByName(this.panel,"name");
        this.uiName = avatarPanel.getChildByName("name");
        //cc.log("initPlayerDisplay 20");
        //this.uiGold = ccui.helper.seekWidgetByName(this.panel,"gold");
        this.uiGold = avatarPanel.getChildByName("gold");
        //cc.log("initPlayerDisplay 21");

        //this.uiHome = ccui.helper.seekWidgetByName(this.panel,"home");
        //this.iconChuong = ccui.helper.seekWidgetByName(this.panel, "imgChuong");
        this.iconChuong = avatarPanel.getChildByName("imgChuong");
        //cc.log("initPlayerDisplay 22");
        this.iconChuong.setVisible(false);
        //this.notify = ccui.helper.seekWidgetByName(this.panel, "notify");
        this.notify = this.panel.getChildByName("notify");

        //cc.log("initPlayerDisplay 23");
        //this.moneyBet = ccui.helper.seekWidgetByName(this.panel, "moneyBet");
        this.moneyBet = this.panel.getChildByName("moneyBet");
        //cc.log("initPlayerDisplay 24");
        this.moneyBet.setString("0");
        this.moneyBet.setVisible(false);
        //this.view = ccui.helper.seekWidgetByName(this.panel, "view");
        this.view = this.panel.getChildByName("view");
        this.view.setVisible(false);
        //this.iconOutRoom = ccui.helper.seekWidgetByName(this.panel,"iconOut");
        this.iconOutRoom = avatarPanel.getChildByName("iconOut");
        this.iconOutRoom.setVisible(false);
        cc.log("initPlayerDisplay 3 " + this.index);
    ...
    setAvatar: function(avatarType){
        //this.avatar.loadTexture("res/common/avatar/Avatar_" + avatarType + ".png",ccui.Widget.PLIST_TEXTURE);
        GuiUtil.changeImage(this.avatar,"res/common/avatar/Avatar_" + avatarType + ".png");
    },

    setAvatarChuan: function(avatarUrl){
        cc.log("avatarUrl: " + avatarUrl);
        //this.avatar.loadTexture(gameUtility.getAvatarPath(avatarUrl),ccui.Widget.PLIST_TEXTURE);
        GuiUtil.changeImage(this.avatar,gameUtility.getAvatarPath(avatarUrl));
    },

    updateTienCuoc: function(){
        if(!this.isChuong()) {
            this.moneyBet.setVisible(true);
            this.moneyBet.setString("Cược: " + StringUtility.standartNumber(BaCay.gameLogic.players[this.index].moneyBet));
        }else{
            this.moneyBet.setVisible(false);
        }
    },
    ...
    moBai: function(){
        ...
        for(var i = 0; i < this.cards.length; i++){
            cards.push(new BaCay.Card(this.cards[i]));
        }

        var bo = new BaCay.BoBaCay(cards);
    ...
    displayDiem: function(){
        ...
        var node;
        if(this.needShadown){
            node = GuiUtil.createSprite("res/CardGame/CommonResource/DiemBoBai/diem3Cay/shadown.png");//GuiUtil.createSprite("res/CardGame/CommonResource/DiemBoBai/diem3Cay/shadown.png");
            var node2 = GuiUtil.createSprite(this.typeString);
            node.addChild(node2);
            node2.setPosition(node.getContentSize().width/2, node.getContentSize().height/2);
        }
        else{
            node = GuiUtil.createSprite(this.typeString);
        }
    ...
    clearBoluot: function() {
        ...
        if(parent.getChildByTag(BaCay.PlayerDisplay.boluottag))
        {
            parent.getChildByTag(BaCay.PlayerDisplay.boluottag).stopAllActions();
            parent.getChildByTag(BaCay.PlayerDisplay.boluottag).setScale(1.1);
            parent.getChildByTag(BaCay.PlayerDisplay.boluottag).runAction(cc.sequence(cc.delayTime(2),cc.fadeOut(.5),cc.removeSelf()));
        }
    },
    ...
    addMoney: function(money, time) {
        ...
        this.updateMoney = function(){
            ...
            cc.log("string: " + StringUtility.standartNumber(BaCay.gameLogic.players[this.index].info["money"]))
            this.uiGold.setString(StringUtility.standartNumber(BaCay.gameLogic.players[this.index].info["money"]));
        }
    ...
    removeNodeMoney: function(){
        cc.log("sap remove this.nodeMoney");
        if(this.nodeMoney){
            cc.log("remove this.nodeMoney");
            this.nodeMoney.removeFromParent();
            this.nodeMoney = null;
        }
    },
    ...
    isChuong: function(){
        return this.index == BaCay.gameLogic.chuongChair;
    }
});
BaCay.PlayerDisplay.sunnodetag = 9211;
BaCay.PlayerDisplay.boluottag = 9215;
