//
BaCay.NanBaiLayer = BaseLayer.extend({
    ctor: function(){
        this._super();
    ...
    customizeGUI2: function(){
        ...
        //this.btnMoLuon = this.customizeButton("btnMoLuon", BaCay.NanBaiLayer.BTN_MO_LUON);
        this.addButton(this,"btnMoLuon", BaCay.NanBaiLayer.BTN_MO_LUON,cc.p(816,520),true,res_CardGame_BaCay + "/moLuon.png",null);
        var startX = 460;
        var startY = 365;

        for(var i = 0; i < 3; i++){
            //this["card"+ i] = this._layout.getChildByName("card" + i);
            this.addSprite(this,"card"+i,cc.p(startX + (180*i),startY),BaCay.res.laBaiChe);
            this["card"+i].setScale(1.3);
            this.cardList.push(this["card" + i]);
            var cardChe = new BaCay.CardSprite(52);
    ...
    initWithQuanBai: function(){
        for(var i = 0; i < this.cards.length; i++){
            //this.cardList[i].setTexture(cc.textureCache.addImage(BaCay.CardSprite.getResource(this.cards[i])));
            GuiUtil.changeSprite(this.cardList[i],BaCay.CardSprite.getResource(this.cards[i]));
        }
    },

    onButtonRelease: function(btn, id){
        switch(id){
            case BaCay.NanBaiLayer.BTN_MO_LUON:
            {
                this.moBai();
            }
                break;
    ...
    onTouchMoved: function(touch, event){
       ...
       // localPoint = target.convertToNodeSpace(localPoint);
        ...
        cc.log("" + target.cardCheList[target.beginCardId].getPosition().x);
...
BaCay.NanBaiLayer.BTN_MO_LUON = 10000;