BaCay.EffectLayer = cc.Layer.extend({
    ctor: function(){
        this._super();
        ...
        this.chatImage = [];
        this.demLui = null;
        for (var i=0; i< BaCay.MAX_PLAYER; i++){
    ...
    chiaBai: function(player, num, stt){
        ...
        var cardImg = null;//new cc.Sprite(BaCay.res.laBaiChe);
        if (cc.spriteFrameCache.getSpriteFrame(BaCay.res.laBaiChe)) {
            cardImg = new cc.Sprite("#" + BaCay.res.laBaiChe);
        }
        else {
            cardImg = new cc.Sprite(BaCay.res.laBaiChe);
        }
        ...
        var timeOffsetAppear = 0.003*timeSlow;
        {
            if(player.index == 0) {
                for ( var i = 0; i < num * 3; i++) {
                    var card = GuiUtil.createSprite(BaCay.res.laBaiChe);//new cc.Sprite(BaCay.res.laBaiChe);
                    card.setScale(0.75);
            ...
            for(var i = 0; i < 3; i++){
                var mycard  = GuiUtil.createSprite(BaCay.res.laBaiChe);//new cc.Sprite(BaCay.res.laBaiChe);
    ...
    addPhatLuong: function(){
        ...
        var effect = GuiUtil.createSprite(BaCay.res.phatLuongPng);//new cc.Sprite(BaCay.res.phatLuongPng);
    ...
    addCaLangSangTien: function(){
        var winSize = SceneMgr.getInstance().getRunningScene().getMainContentSize();
        var effect = GuiUtil.createSprite(BaCay.res.caLangSangTienPng);//new cc.Sprite(BaCay.res.caLangSangTienPng);
        effect.setPosition(winSize.width/2, winSize.height/2 + 100);
        this.addChild(effect);
    ...
    firstTurn: function(player){
        var winSize = SceneMgr.getInstance().getRunningScene().getMainContentSize();
        var layerCard = GuiUtil.createSprite(BaCay.res.cardFirstTurnPng);//new cc.Sprite(BaCay.res.cardFirstTurnPng);
    ...
    moneyFly: function(posSrc,posDst,time,delay,visible)
    {
        ...
        if(rdChip > 6)
            rdChip = 6;
        var chip = GuiUtil.createSprite("res/common/" + "chip/chip"+rdChip+".png");//new cc.Sprite("res/common/" + "chip/chip"+rdChip+".png");
    ...
    anGa: function(from, to){
        ...
        var anGaImg = null;
        if (cc.spriteFrameCache.getSpriteFrame("res/CardGame/BaCay/an_ga.png"))
        {
            anGaImg = new cc.Sprite("#" + "res/CardGame/BaCay/an_ga.png");
        }
        else {
            anGaImg = new cc.Sprite("res/CardGame/BaCay/an_ga.png");
        }
        ...
        for(var i = 0; i < 4; i++){
            ...
            var star = null;
            if (cc.spriteFrameCache.getSpriteFrame("res/CardGame/BaCay/star.png"))
            {
                star = new cc.Sprite("#" + "res/CardGame/BaCay/star.png");
            }
            else {
                star = new cc.Sprite("res/CardGame/BaCay/star.png");
            }
    ...
    addEffectSanhToiCot: function(){
        var winSize = SceneMgr.getInstance().getRunningScene().getMainContentSize();
        effect = GuiUtil.createSprite(Sam.res.nenSanhToiCotPng);//new cc.Sprite(Sam.res.nenSanhToiCotPng);
        effect1 = GuiUtil.createSprite(Sam.res.effectSanhToiCotPng);//new cc.Sprite(Sam.res.effectSanhToiCotPng);
