(function () {
    ...
    var controllerId = uc.adapterManager.adapters.lobby;
    // var adapter = uc.adapterManager.getAdapterByName("lobby");
    var adapter;
    var sendCmds = uc.Caro.sendCmds;
    var receivedCmds = uc.Caro.receivedCmds;
    ...
    CoCaro.CoCaroScene = uc.BaseLayer.extend({
        ctor: function () {
            ...
            this.lastTile = null;
            this.isMarked = false;
            this.gameLogic = new CoCaro.GameLogic();
            adapter = new uc.CaroTestAdapter();
        },
        ...
        listenAdapter: function () {
            ...
            adapter.on(CoCaro.CoCaroCmd.JOIN_ROOM_SUCCESS, this.onJoinRoomSuccess);

            adapter.sendMessage(-CoCaro.CoCaroCmd.JOIN_ROOM_SUCCESS);
            // adapter.on(CoCaro.CoCaroCmd.THONG_TIN_BAN_CHOI, this.onRoomInfo);

            // adapter.on(CoCaro.CoCaroCmd.DANG_KY_THOAT_PHONG, this.onRegisterLeaverRoom);
            // adapter.on(CoCaro.CoCaroCmd.LEAVE_GAME, this.onLeaveGame);
            // adapter.on(CoCaro.CoCaroCmd.NOTIFY_KICK_FROM_ROOM, this.onNotifyKickFromRoom);
            // adapter.on(CoCaro.CoCaroCmd.NEW_USER_JOIN, this.onNewUserJoin);
            // adapter.on(CoCaro.CoCaroCmd.TU_DONG_BAT_DAU, this.onAutoStart);
            // adapter.on(CoCaro.CoCaroCmd.START_GAME, this.onStartGame);
            // adapter.on(CoCaro.CoCaroCmd.TAKE_TURN, this.onTakeTurn);
            // adapter.on(CoCaro.CoCaroCmd.CHANGE_TURN, this.onChangeTurn);
            // adapter.on(CoCaro.CoCaroCmd.KET_THUC, this.onFinish);
            // adapter.on(CoCaro.CoCaroCmd.UPDATE_MATCH, this.onUpdateMatch);
        },

        onJoinRoomSuccess : function (roomInfo) {
                console.log("roomInfo", roomInfo);

            this.gameState = CoCaro.GameState.JOIN_ROOM;
            this.bet = roomInfo.moneyBet;
            this.moneyType = roomInfo.moneyType;
            this.roomId = roomInfo.roomId;
            this.gameId = roomInfo.gameId;
            this.myChair = roomInfo.myChair;
            this.gameAction = roomInfo.gameAction;
            this.activeTimeRemain = roomInfo.countDownTime;

            for(var i = 0; i < CoCaro.GameLogic.MAX_PLAYER; i++)
            {
                var chair = this.convertChair(i);
                if (roomInfo.playerStatus[chair] != 0) {
                    this.players[chair].info = roomInfo.playerInfos[i];
                    this.players[chair].status = roomInfo.playerStatus[i];
                }
            }
        ...
        customizeGUI: function () {
            ...
            if (this.gameLogic.moneyType == MONEY_VIN) {
            ...
            var listener = cc.EventListener.create({
                ...
                onTouchBegan: function (touch, event) {
                    return that.gameLogic.isPlaying && CoCaro.gameLogic.isMyTurn() && !that.isMarked;
                },
                ...
                onTouchEnded: function (touch, event) {
                    ...
                    if (!that.gameLogic.canMark(tile.x, tile.y))
                        return;
                    that.tempMark.setVisible(false);
                    that.isMarked = true;
                    that.lastTile = tile;
                    that.setBoardTile(tile.x, tile.y, that.gameLogic.getCurrentTickType());
            ...
            var mouseListener = cc.EventListener.create({
                ...
                onMouseMove: function (event) {
                    if (!that.gameLogic.isMyTurn() || that.isMarked)
                        return;
